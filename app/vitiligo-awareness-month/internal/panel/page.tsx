...
import {
  getAllDoctorIpledges,
  getAllDoctorPosters,
  getAllDoctorVideos,
  updateUrl,
} from "@/app/actions/other.actions";
...
const Page = () => {
  ...
  const [report, setReport] = useState<"video" | "poster" | "i-pledge">(
    "video"
  );
  ...
  const uploadVideo = async (e: ChangeEvent<HTMLInputElement>, item: any) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/presign?filename=${encodeURIComponent(
          filenameWithFolder
